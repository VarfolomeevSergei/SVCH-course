const { Diagnosis, Patient, Doctor } = require('../models/models'); // Обновленный путь к моделям
...
class DiagnosisController {
    ...
    async findOne(req, res) {
        try {
            const diagnosis = await mongoose.models.Diagnosis.findById(req.params.id)
    ...
    async findAll(req, res) {
        try {
            ...
            const diagnoses = await mongoose.models.Diagnosis.find(whereClause)
                .populate('doctorId', 'id firstName lastName specialization')
                .populate('patientId', 'id firstName lastName phoneNumber')
                .sort({ createdAt: -1 });
    ...
    async update(req, res) {
        try {
            ...
            const diagnosis = await mongoose.models.findById(diagnosisId);
            if (!diagnosis) {
                return res.status(404).json({ message: 'Диагностика не найдена' });
            }

            if (diagnosis.doctorId.toString() !== req.user.doctorId) { 
                return res.status(403).json({ message: 'Доступ запрещён' });
            }

            diagnosis.name = name || diagnosis.name;
            diagnosis.conclusion = conclusion !== undefined ? conclusion : diagnosis.conclusion;
            await diagnosis.save();

            const updatedDiagnosis = await mongoose.models.Diagnosis.findById(diagnosisId)
                .populate('doctorId', 'id firstName lastName specialization')
                .populate('patientId', 'id firstName lastName phoneNumber');
    ...
    async delete(req, res) {
        try {
            ...
            const diagnosis = await mongoose.models.Diagnosis.findById(diagnosisId);
            if (!diagnosis) {
                return res.status(404).json({ message: 'Диагностика не найдена' });
            }
            ...
            await mongoose.models.Diagnosis.remove({_id: diagnosisId});
