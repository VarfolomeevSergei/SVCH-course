...
class ServiceController {
    async create(req, res) {
        try {
            ...
            const existingService = await mongoose.models.Service.findOne({ name });
            if (existingService) {
                return res.status(400).json({ message: 'Услуга с таким названием уже существует' });
            }

            const parsedDepartmentId = departmentId;
            const department = await mongoose.models.Department.findById(parsedDepartmentId);
            if (!department) {
    ...
    async findOne(req, res) {
        try {
            const service = await mongoose.models.Service.findById(req.params.id)
                .populate('departmentId', 'id name')
    ...
    async update(req, res) {
        try {
            ...
            if (departmentId) {
                updatedData.departmentId = departmentId;
            }
            ...
            await Service.findByIdAndUpdate(serviceId, updatedData, { new: true });

            const updatedService = await mongoose.models.Service.findById(serviceId)
                .populate('departmentId', 'id name')
                .select('-createdAt -updatedAt');
    ...
    async delete(req, res) {
        try {
            ...
            await mongoose.models.Service.deleteOne({_id: serviceId});
